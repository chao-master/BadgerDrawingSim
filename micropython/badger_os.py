...
def state_delete(app):
    try:
        os.remove("badger-system/state/{}.json".format(app))
    except OSError:
        pass
...
def state_save(app, data):
    try:
        with open("badger-system/state/{}.json".format(app), "w") as f:
            f.write(json.dumps(data))
            f.flush()
    except OSError:
        import os
        try:
            os.stat("badger-system/state")
        except OSError:
            os.mkdir("badger-system/state")
            state_save(app, data)
...
def state_load(app, defaults):
    try:
        data = json.loads(open("badger-system/state/{}.json".format(app), "r").read())
